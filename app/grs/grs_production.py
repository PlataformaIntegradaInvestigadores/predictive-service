...
class DataPreprocessor:
    ...
    def get_article_authors_map(self) -> Dict[str, Set]:
        ...
        return self.df_author_article.set_index('scopus_id')[
            'authid'
        ].groupby(level=0).apply(set).to_dict()
    
    def get_article_topics_map(self) -> Dict[str, Set]:
        """Mapea artículo -> conjunto de tópicos"""
        return self.df_topic_article.set_index('article_id')[
            'topic'
        ].groupby(level=0).apply(set).to_dict()
...
class GroupIdentifier:
    ...
    def identify_persistent(self, groups_p1: Dict, groups_p2: Dict, 
                           min_papers_total: int = 2) -> Dict:
        """
        ...
        for g1_id in groups.keys():
            ...
            for g2_id in groups.keys():
                if g1_id != g2_id:
                    ...
                    jaccard = SimilarityCalculator.jaccard_similarity(topics_g1, topics_g2)
...
class GroupRecommendationSystem:
    ...
    def generate_all_recommendations(self, k: int = 10,
        ...
        for gid in self.groups.keys():
            recs = self.recommend_for_group(gid, k=k, alpha=alpha, beta=beta, gamma=gamma)
